...
// -----------------------------------------------
// COMPONENT->STAY(RECEIPT) ----------------------
// -----------------------------------------------
export default class InfoStay extends React.Component {
      ...
      if (this.props.booking.confirmed) {
        return (
          <header className="booking-status-header booking-confirmed">
            {this.props.verified ? (
              <h3>{translate(`cx.receipt.booking_status.verified`)}</h3>
            ) : (
              <h3>{translate(`cx.receipt.booking_status.confirmed`)}</h3>
            )}
          </header>
        );
      } else {
        return (
          <header className="booking-status-header booking-unconfirmed">
            <h3>{translate(`cx.receipt.booking_status.unconfirmed`)}</h3>
          </header>
        );
      }
    }
